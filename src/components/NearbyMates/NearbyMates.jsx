...
import PetCard from "../Profile/components/PetCard";
...
const NearbyMates = () => {
    ...
    const fetchUserPets = async () => {
      ...
      try {
        ...
        if (snapshot.exists()) {
          ...
          // If user has pets, set the first one as selected by default
  ...
  const calculateDistance = (lat1, lon1, lat2, lon2) => {
    ...
    // Fetch all pets that are available for mating
    useEffect(() => {
        ...
        try {
          ...
          if (snapshot.exists()) {
            ...
            Object.keys(petsData).forEach(userId => {
              // Skip current user's pets
              if (userId === user?.uid) return;
              
              // Loop through each user's pets
              const userPets = petsData[userId];
              Object.keys(userPets).forEach(petId => {
                const pet = userPets[petId];
                // Only include pets available for mating
                if (pet.availableForMating) {
                  // Add pet location (in real app, this would come from the database)
                  // For demo purposes, we'll generate a random nearby location
                  const petLocation = {
                  ...
                  const ownerData = {
                    id: userId,
                    displayName: "Pet Owner" // In a real app, fetch owner name from users collection
                  };
    ...
    // Filter pets when selectedUserPet or maxDistance changes
    useEffect(() => {
      ...
      // Filter by pet type, opposite gender, and distance
    ...
    // Handle user pet selection change
    ...
    // Handle pet detail click
    const handlePetDetail = (pet) => {
      // You would navigate to a detailed pet profile here
      console.log("View pet details:", pet);
    };
  
    // Open mating request dialog
