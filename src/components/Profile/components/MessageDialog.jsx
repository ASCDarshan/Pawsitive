...
const MessageDialog = ({
  ...
  useEffect(() => {
    ...
    const fetchMatingRequest = async () => {
      try {
        // Check in sent requests first
        const sentRef = ref(database, `matingRequests/sent/${user.uid}/${matingRequestId}`);
        ...
        const receivedRef = ref(database, `matingRequests/received/${user.uid}/${matingRequestId}`);
  ...
  const handleTyping = () => {
    ...
    const typingRef = ref(database, `conversations/${conversationId}/typing/${user.uid}`);
  ...
  // Helper to generate background gradient based on pet type
        ...
        {loading ? (
          ...
          <Box sx={{ 
            ...
            <Typography variant="body2" color="textSecondary" align="center" sx={{ maxWidth: 400 }}>
              This is the beginning of your conversation about the mating request between {senderPet?.name} and {receiverPet?.name}.
            </Typography>
            <Box sx={{ 
              mt: 4, 
              p: 2, 
              ...
              <Button 
                ...
                onClick={() => {
                  setNewMessage("Hello! I'm interested in arranging a meeting for our pets. When would be a good time to discuss the details?");
                }}
              >
                Use Suggestion
              </Button>
                ...
                {groupedMessages[date].map((message, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {!isSender && !showAvatar && <Box sx={{ width: 36, mr: 1 }} />}
                      ...
                      <Box sx={{ 
                        ...
                        <Typography 
                          variant="caption" 
                          color={isSender ? "rgba(255,255,255,0.7)" : "textSecondary"}
                          sx={{ 
                            display: 'block', 
                            mt: 0.5, 
                            mb: 0.5,
                      ...
                      {isSender && !showAvatar && <Box sx={{ width: 36, ml: 1 }} />}
                    </Box>
                  );
                })}
              </Box>
                  ...
                  <Tooltip title="Send message">
                    <span>
                      <IconButton 
                        ...
                        sx={{ 
                          bgcolor: newMessage.trim() ? 'primary.main' : 'rgba(0,0,0,0.08)',
                          color: newMessage.trim() ? 'white' : 'text.disabled',
                          '&:hover': { 
                            bgcolor: newMessage.trim() ? 'primary.dark' : 'rgba(0,0,0,0.08)' 
                          },
