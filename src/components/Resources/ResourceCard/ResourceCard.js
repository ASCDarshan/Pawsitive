...
const ResourceCard = ({ resource, onResourceUpdated }) => {
  ...
  useEffect(() => {
    // Only proceed if we have hours data
    ...
    const checkIfOpen = () => {
      ...
      // Get the appropriate hours string for today
      if (Array.isArray(resource.hours)) {
        // Find today's hours in the array
      ...
      if (match) {
        ...
        // Couldn't parse times, default to closed
        setIsOpen(false);
      }
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      // Update user's like in Firestore
      const userLikeRef = doc(db, "resources", resource.id, "likes", auth.currentUser.uid);
    ...
    try {
      ...
      try {
        if (editComment) {
          ...
          const commentRef = ref(database, `resources/${resource.id}/comments/${editComment.id}`);
          await update(commentRef, { text: newComment });
          
          // Also update in userComments
          const userCommentRef = ref(database, `userComments/${auth.currentUser.uid}/${resource.id}/${editComment.id}`);
          ...
          // Also add to userComments
          const commentId = newCommentRef.key;
          const userCommentRef = ref(database, `userComments/${auth.currentUser.uid}/${resource.id}/${commentId}`);
        ...
        console.warn("Realtime DB comment update failed, trying Firestore:", rtdbError);
      }
      ...
      if (editComment) {
        ...
        const docRef = await addDoc(collection(db, `resources/${resource.id}/comments`), commentData);
        
        // Also add to user's comments collection for easier retrieval
  ...
  {/* Add the open/closed status indicator here */}
{checkedOpenStatus && (
  <div className={`absolute top-0 right-0 ${isOpen ? 'bg-green-500' : 'bg-red-500'} text-white px-2 py-1 text-sm rounded-bl`}>
  ...
  const handleDeleteComment = async (commentId) => {
    ...
    try {
      // Try Realtime Database first
      try {
        const commentRef = ref(database, `resources/${resource.id}/comments/${commentId}`);
        ...
        console.warn("Realtime DB comment delete failed, trying Firestore:", rtdbError);
      }
      
      // Fallback to Firestore
      await deleteDoc(doc(db, `resources/${resource.id}/comments`, commentId));
      
      // Also try to delete from user's comments collection
      try {
  ...
  const handleViewDetails = () => {
    ...
    if (resource.id) {
      // Navigate to the resource details page with the ID in the URL and full resource data in state
      navigate(`/resource-details/${resource.id}`, {
        state: { resourceData: resource }
      });
    } else if (resource.place_id) {
      // If using place_id instead of id (for Google Maps resources)
    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden transition duration-300 hover:shadow-xl max-w-sm mx-auto border border-lavender-300">
      ...
      <div className="p-4">
        ...
        <div className="flex justify-between items-center">
          <button 
            onClick={handleLike} 
            className={`flex items-center ${liked ? "text-red-500" : "text-lavender-600"} hover:text-red-500 transition-colors`}
            ...
            <div className="p-4">
              ...
              {comments.length > 0 ? (
                <div className="mt-4 space-y-4">
                  <h3 className="font-medium text-lavender-900">Comments ({comments.length})</h3>
                ...
                <p className="text-gray-500 mt-4 text-center">No comments yet. Be the first to comment!</p>
              )}
            </div>
